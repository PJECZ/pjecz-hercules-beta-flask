...
from pjecz_hercules_beta_flask.blueprints.ofi_documentos_destinatarios.forms import OfiDocumentoDestinatarioForm
...
def new_with_ofi_documento(ofi_documento_id):
    ...
    if ofi_documento.esta_archivado:
        flash("No se pueden agregar destinatarios a un documento archivado", "warning")
    ...
    if ofi_documento.esta_cancelado:
        flash("No se pueden agregar destinatarios a un documento cancelado", "warning")
        return redirect(url_for("ofi_documentos.detail", ofi_documento_id=ofi_documento_id))
    ...
    if not (current_user.id == propietario.id or current_user.autoridad.id == autoridad.id):
        flash("No tiene permiso para agregar destinatarios a este documento", "danger")
        return redirect(url_for("ofi_documentos.detail", ofi_documento_id=ofi_documento_id))
